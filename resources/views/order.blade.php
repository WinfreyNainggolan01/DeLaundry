...
<script>
    ...
    const items = []; // Store items added to the order summary
    ...
    document.getElementById('add-item-btn').addEventListener('click', function() {
        ...
        if (currentEditIndex === -1) { // If not editing, add a new item
            // Create new item object
            const newItem = { name: itemName, quantity: quantity, category: category, note: note };
            items.push(newItem);
        } else { // If editing, update the existing item
            items[currentEditIndex] = { name: itemName, quantity: quantity, category: category, note: note };
            currentEditIndex = -1; // Reset edit index after editing
        }
    ...
    function updateOrderSummary() {
        ...
        orderSummaryBody.innerHTML = ''; // Clear current summary
    ...
    <td class="px-4 py-2 flex items-center justify-center"> <!-- Flexbox untuk menempatkan ikon sejajar -->
        <img src="img/editing.png" alt="Edit" class="cursor-pointer mr-2" onclick="editItem(${index})" style="width: 24px; height: 24px;">
        <img src="img/delete.png" alt="Delete" class="cursor-pointer" onclick="deleteItem(${index})" style="width: 24px; height: 24px;">
    </td>
</tr>

            `;
            orderSummaryBody.insertAdjacentHTML('beforeend', newRow);
        });
    }
